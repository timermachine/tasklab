...
function renderStep(step, runState) {
  ...
  if (step.type === 'hitl') {
    ...
    return `
      <div class="step-card bg-gray-900 border border-gray-800 border-l-4 border-l-violet-400 rounded-xl mb-2 overflow-hidden status-manual" id="sc-${step.idx}" data-step-idx="${step.idx}" data-status="manual">
  ...
  const failOutput = status === 'failed' && runStep?.output
    ? `<div class="px-3 pb-3 pl-[38px]">
        <pre class="text-xs text-rose-300 bg-rose-950/40 border border-rose-400/20 rounded-lg p-3 whitespace-pre-wrap overflow-x-auto font-mono leading-relaxed">${esc(runStep.output)}</pre>
      </div>`
    : '';
  return `
    <div class="step-card bg-gray-900 border border-gray-800 rounded-xl mb-2 overflow-hidden status-${esc(status)}" id="sc-${step.idx}" data-step-idx="${step.idx}" data-status="${esc(status)}">
      <div class="step-header flex items-start justify-between gap-3 p-3">
        <div class="flex items-start gap-2 min-w-0 flex-1">
          <span class="snum flex-shrink-0 w-[22px] h-[22px] rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center text-[11px] font-bold text-gray-400">${n}</span>
...
function buildHtml({ task, plan, manifest, steps, projectRoot, taskDirRel, runState }) {
...
<head>
  ...
  ${runActive ? '<meta http-equiv="refresh" content="2" />' : ''}
...
<div class="max-w-3xl mx-auto px-4 py-6 pb-16">
  ...
  <div class="mb-5">
    ...
    ${runState ? `<div class="mt-3 text-xs ${runFailed ? 'text-rose-300' : runDone ? 'text-emerald-300' : 'text-blue-300'}">
...
function main() {
  ...
  const html = buildHtml({ task, plan, manifest, steps, projectRoot, taskDirRel, runState });
