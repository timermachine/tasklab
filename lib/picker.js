...
async function picker() {
  ...
  items.push({ type: 'header', label: '─── Your tasks  ./tasklab/tasks/ ' + '─'.repeat(29) });
  if (localTasks.length === 0) {
    items.push({ type: 'empty', label: '  (none — run `tasklab init` to create your first task)' });
  } else {
