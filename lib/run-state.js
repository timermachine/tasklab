...
function markScriptSuccess(projectRoot, state, scriptPath, output) {
  const patch = {
    status: 'success',
    completed_at: new Date().toISOString(),
    exit_code: 0,
  };
  if (output) patch.output = output.slice(0, 4000);
  return writeRunState(projectRoot, updateScript(state, scriptPath, patch));
}

function markScriptFailed(projectRoot, state, scriptPath, exitCode, output) {
  const patch = {
    status: 'failed',
    completed_at: new Date().toISOString(),
  ...
  if (output) patch.output = output.slice(0, 4000);
  const next = updateScript(state, scriptPath, patch);
